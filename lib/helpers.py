...
def _improve_factor(interval):
    ...
    :rtype: float
    """
    improve_factor = np.array([[1, 1.14],
                               [2, 1.07],
                               [3, 1.04],
                               [4, 1.03],
                               [5, 1.0],
    ...
    return np.interp(interval, improve_factor[:, 0], improve_factor[:, 1])
...
def calculate_u_w(file_input, duration_steps, measurement_period, series_kind):
    ...
    for duration_index in duration_steps:
        ...
        sum_series = ts.rolling(duration).sum()
        events[MAX_OSUM] = events.apply(lambda event: sum_series.loc[event['start']:event['end']].max() * improve,
                                        axis=1)

        # ev = pd.IntervalIndex.from_arrays(events['start'].values, events['end'].values + pd.Timedelta(base_frequency), closed='left')
        # test = sum_series.groupby(pd.cut(sum_series.index, ev)).max() * improve
